...
export const Students = () => {
  ...
  return(
    <Box className={cls.content}>
        ...
        <ul className={cls.list}>
          {
            statistics?.map((stat, index) => (
              <li className={cls.item} key={index}>
                ...
                <h3 className={cls.subtitle}>{stat?.title}</h3>
                <Box className={cls.wrapperText}>
                  <span className={cls.count}>{stat?.count}</span>
                  <span className={cls.measurement}>{stat?.measurement}</span>
                </Box>
              </li>
            ))
          }
      ...
      {/*  Modals  */}
      <AddModal title="Регистрация ученика" handleAccept={handleSubmit(onSubmit)} register={register} isOpen={isOpen} onClose={handleClose} >
        <FormControl as='form'>
      ...
      <EditModal title="Изменение данные ученика" handleAccept={handleSubmit(handleUpdateUser)} isOpen={isOpenEdit} register={register} onClose={handleEditClose}>
        <FormControl as='form'>
          <Box className={cls.wrapper}>
            <Inputs
              type="text"
